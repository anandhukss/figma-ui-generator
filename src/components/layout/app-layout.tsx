...
export function AppLayout({ children, currentPath, navigate }: AppLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-background">
      {open && <button className="fixed inset-0 z-30 bg-foreground/30 lg:hidden" aria-label="Close navigation" onClick={() => setOpen(false)} />}
      <aside className={cn('fixed inset-y-0 left-0 z-40 flex w-64 flex-col bg-sidebar text-sidebar-foreground transition-transform lg:translate-x-0', open ? 'translate-x-0' : '-translate-x-full')}>
        ...
        <nav className="flex-1 px-4 py-4"><p className="mb-3 px-3 text-xs font-semibold uppercase tracking-widest text-sidebar-muted">Banking</p><div className="space-y-1">{navItems.map(({ label, href, icon: Icon }) => <button key={href} onClick={() => go(href)} className={cn('flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition', currentPath === href ? 'bg-sidebar-active text-sidebar-active-foreground' : 'text-sidebar-muted hover:bg-sidebar-foreground/10 hover:text-sidebar-foreground')}><Icon className="size-4" />{label}</button>)}</div></nav>
